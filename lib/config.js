...
function normalizeMinifierConfig(config) {
	// If minifyCSS is false or a function, we don't need to touch it
	if (!config || !config.minifyCSS || typeof config.minifyCSS === 'function') {
		return config;
	}

	const customConfig = config && typeof config.minifyCSS === 'object' ? config.minifyCSS : undefined;
	const minifyCSS = createMinifyCSS(customConfig);
